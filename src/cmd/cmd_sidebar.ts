  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      const hasContent = subEntries.some(
        (e) =>
          e.endsWith(".md") ||
          (fs.statSync(path.join(fullPath, entry.name, e)).isDirectory() && !EXCLUDE_DIRS.includes(e))
      );
      ...
      if (hasContent) {
        ...
        const subItems = generateSidebar(rootDir, relativePath, currentDepth + 1, maxDepth);
        items.push(...subItems);
      }
  ...
  const program = new Command("sidebar")
    ...
    .action((options: SidebarOptions) => {
      try {
        ...
        const maxDepth = options.depth === Number.MAX_SAFE_INTEGER ? Infinity : options.depth;
