...
async function parseMarkdownMetadata(filePath: string): Promise<void> {
  try {
    ...
    console.log(
      `  年-月-日      : ${parsedPermalink.year}-${parsedPermalink.month.toString().padStart(2, "0")}-${parsedPermalink.day.toString().padStart(2, "0")}`
    );
    console.log(
      `  时:分:秒.毫秒 : ${parsedPermalink.hours.toString().padStart(2, "0")}:${parsedPermalink.minutes.toString().padStart(2, "0")}:${parsedPermalink.seconds.toString().padStart(2, "0")}.${parsedPermalink.milliseconds.toString().padStart(3, "0")}`
    );
  } catch (err) {
