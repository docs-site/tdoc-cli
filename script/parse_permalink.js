...
function testParsePermalink() {
  ...
  testLinks.forEach((link, index) => {
    try {
      ...
      console.log(`  年-月-日: ${result.year}-${result.month.toString().padStart(2, '0')}-${result.day.toString().padStart(2, '0')}`);
      console.log(`  时:分:秒.毫秒: ${result.hours.toString().padStart(2, '0')}:${result.minutes.toString().padStart(2, '0')}:${result.seconds.toString().padStart(2, '0')}.${result.milliseconds.toString().padStart(3, '0')}`);
